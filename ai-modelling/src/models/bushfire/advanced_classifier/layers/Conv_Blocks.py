...
class Inception_Block_V1(nn.Module):  # handle different spatial sizes
    def __init__(self, in_channels, out_channels, group=1, num_kernels=6, init_weight=True):
        ...
        for i in range(self.num_kernels):
            kernels.append(
                nn.Conv2d(in_channels, out_channels, kernel_size=2 * i + 1, padding=i, groups=group)
            )
        self.kernels = nn.ModuleList(kernels)  # register kernels by ModuleList
        if init_weight:
            self._initialize_weights()
